<!DOCTYPE html>

<html lang="en">

<head>

    <meta charset="UTF-8">

    <meta http-equiv="X-UA-Compatible" content="IE=edge">

    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title>Webpage</title>

    <link rel="stylesheet" href="mycss_web.css">

    <link rel="stylesheet" href="mycss_tablet.css">

    <style>
        body {

            font-family: Verdana, sans-serif;

            margin: 0;

        }

        .clear {

            clear: both;

        }

        main {

            margin: 10px;

        }

        nav ul {

            list-style-type: none;

            margin: 0;

            display: flex;

            flex-direction: row;

            justify-self: start;

            align-items: center;

            padding: 0;

            overflow: hidden;

            background-color: #000;

        }

        nav li a {

            display: block;

            color: white;

            text-align: center;

            padding: 14px 16px;

            text-decoration: none;

        }

        nav li a:hover {

            background-color: rgb(240, 240, 240);

            color: #000;

            text-decoration: none;

        }

        img {

            float: left;

            margin: 0 12px;

        }

        a {

            text-decoration: none;

            color: blue;

            cursor: pointer;

        }

        a:hover {

            text-decoration: underline;

            cursor: pointer;

            color: #000;

        }

        footer {

            background-color: #000;

            color: white;

            padding: 10px;

            text-align: center;

        }

        input {

            width: 99%;

        }

        table {

            width: 100%;

        }

        p {

            padding-left: 5px;

            padding-right: 5px;

        }

        div {

            padding: 0px;

        }

        #section_eight>form>table>tbody>tr:nth-child(3)>td:nth-child(3)>button {

            width: 100%;

            background-color: Black;

            color: WHITE;

            border: none;

        }

        .text-center {

            text-align: center;

        }

        .row {

            display: -webkit-box;

            display: -ms-flexbox;

            display: flex;

            -ms-flex-wrap: wrap;

            flex-wrap: wrap;

        }

        .col-2 {

            width: 16.66%;

        }

        .col-3 {

            width: 25%;

        }

        .col-4 {

            width: 33.33%;

        }

        .col-6 {

            width: 50%;

        }

        .col-12 {

            width: 100%;

        }

        .col-8 {

            width: 66.66%;

        }

        .col-9 {

            width: 75%;

        }

        .pl-30 {

            padding-left: 30px;

        }

        .p-30 {

            padding: 30px;

        }

        ------------mycss_tablet.css----------- @media screen and (max-width: 768px) {

            .col-xs-3 {

                width: 25%;

            }

            .col-xs-4 {

                width: 33.33%;

            }

            .col-xs-6 {

                width: 50%;

            }

            .col-xs-12 {

                width: 100%;

            }

        }

        @media screen and (max-width: 768px) {

            .topnav a:not(:first-child) {

                display: none;

            }

            .topnav a.icon {

                float: right;

                display: block;

            }

        }

        @media screen and (max-width: 768px) {

            .topnav.responsive {

                position: relative;

            }

            .topnav.responsive .icon {

                position: absolute;

                right: 0;

                top: 0;

            }

            .topnav.responsive a {

                float: none;

                display: block;

                text-align: left;

            }

        }
    </style>

</head>

<body>

    <header>

        <nav>

            <ul>

                <li><a href="#section_one">About Me </a></li>

                <li><a href="#section_two">Education </a></li>

                <li><a href="#section_three">Programming Knowledge </a></li>

                <li><a href="#section_four">Interests/Hobbies </a></li>

                <li><a href="#section_five">Favorite Media </a></li>

                <li><a href="#section_six">Favorite Website </a></li>

                <li><a href="#section_seven">My Projects </a></li>

            </ul>

        </nav>

    </header>

    <main>

        <h1 class="text-center">Daniel's Profile</h1>

        <hr>

        <div id="section_one">

            <div class="row">

                <div class="col-3 col-xs-12">

                    <img src="data:image/png;base64," alt="Online hosted input device image" Style="width:100%;border-radius: 100%;">

                </div>

                <div class="col-9 col-xs-12">

                    <h3 class="pl-30">Daniel Perrone pursuing an <span>Information Technology</span> degree at St. John's University.</h3>

                    <hr class="hrpl-30" style="margin-left: 32px;">

                    <p class="pl-30"> <strong>About me:</strong> I am a 23 year old student at St. John's University. I will be graduating next May with a Information Technology degree. I am half Arab and the other half is mostly Italian with other European ethnicities in the mix.

                    </p>

                </div>

            </div>

            <div class="clear"></div>

        </div>

        <div id="section_two">

            <h2 class="text-center">Education:</h2>

            <hr>

            <div class="row">

                <div class="col-9 col-xs-12">

                    <p>I graduated from Kellenberg Memorial High School in 2017. I am now a senior at St. John's University. I attended Pennsylvania State University for my first year of college and then transferred.</p>

                </div>

                <div class="col-3 col-xs-12">


                </div>

                <div id="section_three">

                    <h2 class="text-center">Work Experience:</h2>

                    <hr>


                    <div class="row">

                        <div class="col-2 col-xs-12"></div>

                        <div class="col-4 col-xs-12">

                            <ul>

                                <li>2 years in retail at Petco as a cashier and dog trainer</li>

                                <li>6 months as an IT intern at Maspeth Federal Savings</li>

                            </ul>

                        </div>

                        <div id="section_four">

                            <h2 class="text-center">Programming knowledge:</h2>

                            <div class="row">

                                <div class="col-0 col-xs-12">

                                    <hr class="hrpl-30">

                                    <p>I had an html class in my Junior year of high school. I also took a Python class at PennState and 2 Java courses at St. John's. I am currently learning C++ on my own time. All of these classes were very beginner oriented and I am not experienced beyond very basic coding.</p>

                                </div>

                                <div class="col-6 col-xs-12">

                                    <h3>Language Experience: </h3>

                                    <hr class="hrpl-30">

                                    <ol type="i">

                                        <li>html - 2 months</li>

                                        <li>Java - 8 months</li>

                                        <li>Python - 4 months</li>

                                        <li>C++ - 1 month</li>

                                    </ol>

                                </div>

                            </div>

                        </div>

                        <div id="section_five">

                            <h2 class="text-center">Interests/Hobbies:</h2>

                            <hr>

                            <div class="row">

                                <div class="col-8 col-xs-12">

                                    <p>On my free time I am typically playing video games with friends online. I also enjoy listening to music. I've played piano for a few years and am now trying to teach myself the guitar. I am also interested in painting on canvas and drawing sketches.</p>

                                </div>

                                <div class="col-4 col-xs-12">

                                </div>

                                <div id="section_six">

                                    <h2 class="text-center">Favorite Media:</h2>

                                    <hr>

                                    <div class="row">

                                        <div class="col-1 col-xs-12">

                                            <div class="p-30">

                                                <p class="text-center">Favorite Song: Baby - Oh Wonder</p>

                                                <iframe width="560" height="315" src="https://www.youtube.com/embed/UcG5MEnbOyA?autoplay=0">
</iframe>

                                                </video>

                                            </div>

                                        </div>

                                        <div class="col-1 col-xs-12">

                                            <div class="p-30">

                                                <p class="text-center">Favorite Movie: Surf's Up</p>

                                                <iframe width="560" height="315" src="https://www.youtube.com/embed/7mPdQRXFiPA?autoplay=0" target="_blank" frameborder="0" allowfullscreen></iframe>
                                                </iframe>

                                                </video>

                                            </div>

                                        </div>

                                        <div class="col-1 col-xs-12">

                                            <div class="p-30">

                                                <p class="text-center">Favorite TV Show: Avatar: The Last Airbender</p>

                                                <iframe width="560" height="315" src="https://www.youtube.com/embed/dMnDkFktGF4?autoplay=0" target="_blank" frameborder="0" allowfullscreen></iframe>
                                                </iframe>

                                                </video>

                                            </div>

                                        </div>

                                    </div>

                                    <div id="section_seven">

                                        <h2 class="text-center">Favorite Websites:</h2>

                                        <hr class="hrpl-30" style="margin-left: auto;">

                                        <div class="row">

                                            <div class="col-6 col-xs-12">

                                                <h4 class="text-center">Reddit:</h4>

                                                <p>This is a social media website where you can follow specific "subreddits" of any topic you can think of.

                                                </p>

                                                <a href="https://www.reddit.com/" style="display: flow-root;" class="text-center" target="_blank">Reddit.com</a>

                                            </div>

                                            <div class="col-6 col-xs-12">

                                                <h4 class="text-center">Youtube:</h4>

                                                <p>This is a video sharing website where you can upload anything you want for the world to see. You can watch videos of your favorite music, movies, and youtube creators.

                                                </p>

                                                <a href="https://www.youtube.com/" target="_blank" style="display: flow-root;" class="text-center">Youtube.com</a>

                                            </div>

                                        </div>

                                    </div><br/>

                                    <div id="section_eight">

                                        <h2 class="text-center">Project Progression:</h2>

                                        <hr class="hrpl-30" style="margin-left: auto;">

                                        <form action="">

                                            <table border="2">

                                                <tr>

                                                    <th>Index</th>

                                                    <th>Project</th>

                                                    <th>Progress</th>

                                                </tr>

                                                <tr>

                                                    <td>1</td>

                                                    <td>Milestone 1</td>

                                                    <td><input type="text" name="" id=""></td>

                                                </tr>

                                                <tr>

                                                    <td>2</td>

                                                    <td>Milestone 2</td>

                                                    <td><input type="text" name="" id=""> <button type="submit">Change</button></td>

                                                </tr>

                                            </table>

                                        </form>

                                    </div>

    </main> <br><br><br>


</body>

</html>
