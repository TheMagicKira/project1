# themagickira.github.io
<!DOCTYPE html>
<html

<head>
  <title>Daniel's Profile</title>
  <style>
    body {
    }
    main {
    }
    nav ul {
      list-style-type: none;
      margin: 0;
      display: flex;
      flex-direction: row;
      justify-self: start;
      align-items: center;
      padding: 0;
      overflow: hidden;
      background-color: #000;
    }

    nav li a {
      display: block;
      color: black;
      text-align: center;
      padding: px;
      text-decoration: none;
    }

    nav li a:hover {
      background-color: rgb(244, 244, 244);
      color: #000;
      text-decoration: none;
    }
    img {
      float: left;
      margin: 0 1px;
    }

    a {
      text-decoration: none;
      color: blue;
      cursor: pointer;
    }

    a:hover {
      text-decoration: underline;
      cursor: pointer;
      color: #000;
    }

    footer {
      background-color: #000;
      color: white;
      padding: 10px;
      text-align: center;
    }

  </style>
</head>

<body>
  <header>

    <nav>
      </ul>
        <li><a href="#section_one">About Me </a></li>
        <li><a href="#section_two">Education </a></li>
        <li><a href="#section_three">Programming Knowledge </a></li>
        <li><a href="#section_four">Interests/Hobbies</a></li>
        <li><a href="#section_five">Favorite Media</a></li>
        <li><a href="#section_six">Favorite Website</a></li>
        <li><a href="#section_seven">My Projects</a></li>
      </ul>
    </nav>
  </header>
  <main>
    <h1>Daniel's Profile</h1>
    <h2>Daniel Perrone pursuing an <span>Information Technology</span> degree at St. John's University.</h3>
    <div id="section_one">
      <img src="data:image/png;base64,"
        height="150" width="150">

      <p> <strong> About me:</strong> I am a 23 year old student at St. John's University. I will be graduating next May with a Information Technology degree. I am half Arab and the other half is mostly Italian with other European ethnicities in the mix.
      </p>
      <div class="clear"></div>
    </div>

    <div id="section_two">
      <h2>Education:</h2>
      <p>I graduated from Kellenberg Memorial High School in 2017. I am now a senior at St. John's University. I attended Pennsylvania State University for my first year of college and then transferred.</p>

    <div id="section_three">
      <h2>Work Experience:</h2>

      <ul>
        <li>2 years in retail at Petco as a cashier and dog trainer</li>
        <li>6 months as an IT intern at Maspeth Federal Savings</li>
      </ul>
    </div>

    <div id="section_four">
      <h2>Programming knowledge:</h2>
      <p>I had an html class in my Junior year of high school. I also took a Python class at PennState and 2 Java courses at St. John's. I am currently learning C++ on my own time. All of these classes were very beginner oriented and I am not experienced beyond very basic coding.</p>

      <h3>Language Experience: </h3>
      <ol type="i">
        <li>html - 2 months</li>
        <li>Java - 8 months</li>
        <li>Python - 4 months</li>
        <li>C++ - 1 month</li>
      </ol>
    </div>

    <div id="section_five">
      <h2>Interests/Hobbies</h2>
      <p>On my free time I am typically playing video games with friends online. I also enjoy listening to music. I've played piano for a few years and am now trying to teach myself the guitar. I am also interested in painting on canvas and drawing sketches.</p>

      <div id="section_six">
      <h2>Favorite Media:</h2>
      <p>Favorite Song: Baby - Oh Wonder</p>
      <br />
      <iframe width="560" height="315" src="https://www.youtube.com/embed/UcG5MEnbOyA?autoplay=0" target="_blank" frameborder="0" allowfullscreen></iframe>
      </iframe>

      <p>Favorite Movie: Surf's Up</p>
        <br />
      <iframe width="560" height="315" src="https://www.youtube.com/embed/7mPdQRXFiPA?autoplay=0" target="_blank" frameborder="0" allowfullscreen></iframe>
      </iframe>

      <p>Favorite TV Show: Avatar: The Last Airbender</p>
      <br />
      <iframe width="560" height="315" src="https://www.youtube.com/embed/dMnDkFktGF4?autoplay=0" target="_blank" frameborder="0" allowfullscreen></iframe>
      </iframe>
    </div>


  <div id="section_seven">
      <h2>Favorite Websites:</h2>
      <p>Reddit: This is a social media website where you can follow specific "subreddits" of any topic you can think of.
      </p>
      <a href="https://www.Reddit.com" target="_blank">>Reddit.com</a> <br>

      <p>Youtube: This is a video sharing website where you can upload anything you want for the world to see. You can watch videos of your favorite music, movies, aand youtube creators.
      </p>
      <a href="https://www.Youtube.com" target="_blank">>Youtube.com</a>

    </div>

    <div id="section_eight">
      <h2>This is section about my projects in this course </h2>
      <h3>About my projects in this course</h3>
      <h4>My projects in this course are below</h4>

      <form action="">

        <table>

          <tr>

            <th>Index</th>
            <th>Project</th>
            <th>Progress</th>
          </tr>
          <tr>

            <td>1</td>
            <td>Project 1</td>
            <td><input type="text" name="" id=""></td>

          </tr>

        </table>
      </form>
    </div>
  </main> <br><br><br>

</body>
</html>
